...
**[Optimize Vec allocations with manual into_iter()]**
**Learning:** Initializing an iterator and unnecessarily using `.collect::<Vec<_>>().into_iter()` causes unnecessary heap reallocations just to yield an iterator again. If the source is already an iterator or can be easily consumed as one, taking ownership and chaining directly prevents allocations in hot loops.
**Action:** When a method needs an iterator (`impl Iterator<Item = T>`), instead of taking a reference to a `Vec` or collecting into a `Vec` and calling `.into_iter()`, use `.into_iter()` directly from the mapping logic or update signatures to accept `impl Iterator` to avoid heap allocations.
